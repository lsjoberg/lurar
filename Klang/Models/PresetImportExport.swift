...
private let log = Logger(subsystem: "se.linus.klang", category: "PresetIO")

/// Save/Open-panel-driven export and import for user presets.
///
/// On-disk format:
///   - `.klangpreset`  — one preset, encoded as a single JSON object
///   - `.klangpresets` — multiple presets, encoded as a JSON array (same
...
enum PresetImportExport {
    static let singleExt = "klangpreset"
    static let bundleExt = "klangpresets"
    ...
    static func exportLibrary(_ presets: [EQPreset]) {
        ...
        panel.nameFieldStringValue = "Klang Presets.\(bundleExt)"
        panel.canCreateDirectories = true
        guard panel.runModal() == .OK, let url = panel.url else { return }
        do {
